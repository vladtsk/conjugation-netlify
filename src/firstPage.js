import {
  buildTenseSelectPopup,
  buildPhraseTypeSelectPopup,
} from "./firstPageSelectPopups.js";
...
export async function buildFirstPage(userId) {
  ...
  currentTense.textContent =
    userId && storedTense ? storedTense : "present (le présent de l'indicatif)";
  ...
  buildPhraseTypeSelectPopup();
  const selectPhrTypeContainer = document.querySelector(
    ".selectPhrTypeContainer"
  );
  ...
  firstTimeDiv.innerHTML =
    "First time here? <p><span class='firstTimeBtn'>Click here</span> to learn how to use the app!</p>";
  ...
  selectDiv.addEventListener("click", (event) => {
    const tenseSelectPopupContainer = document.querySelector(
      ".tenseSelectPopupContainer"
    );
